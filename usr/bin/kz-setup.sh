...
if $GUI && $APT && gsettings list-keys gnome-shell-extension-dashtodock &> /dev/null && ! [[ $(uname --kernel-version) =~ 'Ubuntu' ]] ; then gnome-extensions enable dash-to-dock@micxgx.gmail.com ; fi
if $GUI && $APT && gsettings list-keys gnome-shell-extension-no-overview &> /dev/null ; then gsettings set org.gnome.shell.extensions.dash-to-dock disable-overview-on-startup true ; fi
...
# gnome-shell-extension-dashtodock as a virtual package
# which has been replaced by gnome-shell-extension-ubuntu-dock.
if $GUI && $APT && gsettings list-keys gnome-shell-extension-dashtodock &> /dev/null && ! [[ $(uname --kernel-version) =~ 'Ubuntu' ]] ; then gnome-extensions disable dash-to-dock@micxgx.gmail.com ; fi
if $GUI && $APT && gsettings list-keys gnome-shell-extension-no-overview &> /dev/null ; then gsettings reset org.gnome.shell.extensions.dash-to-dock disable-overview-on-startup  ; fi
