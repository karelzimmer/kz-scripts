...
if grep debian /etc/os-release && gnome-extensions info dash-to-dock@micxgx.gmail.com; then gsettings set org.gnome.shell.extensions.dash-to-dock disable-overview-on-startup true; fi
...
if grep debian /etc/os-release && [[ ${DISPLAY-} ]]; then xdg-mime default gdebi.desktop application/vnd.debian.binary-package; fi
if grep rhel /etc/os-release && [[ ${DISPLAY-} ]]; then echo 'App gdebi is not available.'; fi
...
if grep debian /etc/os-release && [[ ${DISPLAY-} ]]; then kz-desktop --addaft=spotify; fi
if grep rhel /etc/os-release && [[ ${DISPLAY-} ]]; then kz-desktop --addaft=kz-spotify; fi

# Reset spotify on pc01 pc02 pc06 pc07
# Music and podcasts.
# Web app: https://open.spotify.com
if grep debian /etc/os-release && [[ ${DISPLAY-} ]]; then kz-desktop --delete=spotify; fi
if grep rhel /etc/os-release && [[ ${DISPLAY-} ]]; then kz-desktop --delete=kz-spotify; fi
