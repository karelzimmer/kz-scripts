...
# Remove 7zip from *
# File archiver.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes p7zip-full; fi
...
# Configuration management, deployment, and task execution.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes ansible; fi
...
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes anydesk; fi
...
# Backups/snapshots.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes backintime-qt; fi
...
# Delete files.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes bleachbit; fi
...
# E-book manager.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes calibre; fi
...
# Web console.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes cockpit; fi
...
# Remove cups from *
# Common UNIX Printing System.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes cups; fi
...
# Printer backend.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes cups-backend-bjnp; fi
if $DESKTOP_ENVIRONMENT && $RPM_SYSTEM; then echo 'The cups-backend-bjnp app is not available.'; fi
...
# Desktop dock.
# Reboot required!
if $DESKTOP_ENVIRONMENT && $DEBIAN; then sudo apt-get remove --purge --assume-yes gnome-shell-extension-dashtodock; fi
...
# Read and write meta information.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes libimage-exiftool-perl; fi
...
# Simulate superuser privileges.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes fakeroot; fi
...
# Find duplicate files.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes fdupes; fi
...
# Remove gdebi from *
# View and install deb files.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes gdebi; fi
if $DESKTOP_ENVIRONMENT && $RPM_SYSTEM; then echo 'The gdebi app is not available.'; fi
...
# GNU Internationalization.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes gettext; fi
...
# GNU Image Manipulation Program.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes gimp gimp-help-en gimp-help-nl; fi
...
# Distributed revision control system.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes git; fi
...
# Gmail for e-mail.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes gnome-gmail; fi
if $DESKTOP_ENVIRONMENT && $RPM_SYSTEM; then echo 'The gnome-gmail app is not available.'; fi
...
# Adjust advanced settings.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes gnome-tweaks; fi
if $DESKTOP_ENVIRONMENT && $RPM_SYSTEM; then sudo dnf remove --assumeyes gnome-tweaks; fi

# Install google-chrome on *
# Web browser.
# Direct deb download: wget https://dl.google.com/linux/direct/google-chrome-stable_current_amd64.deb
...
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes google-chrome-stable chrome-gnome-shell; fi
...
# Explore the planet.
# Direct deb download: wget https://dl.google.com/linux/direct/google-earth-pro-stable_current_amd64.deb
...
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes google-earth-pro-stable; fi
...
# Video-dvd ripper and transcoder.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes handbrake; fi
if $DESKTOP_ENVIRONMENT && $RPM_SYSTEM; then echo 'The handbrake app is not available.'; fi
...
# Process viewer.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes htop; fi
...
# Slideshow maker.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes imagination; fi
if $DESKTOP_ENVIRONMENT && $RPM_SYSTEM; then echo 'The imagination app is not available.'; fi
...
# JSON processor.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes jq; fi
...
# Image manipulation.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes krita; fi
if $DESKTOP_ENVIRONMENT && $RPM_SYSTEM; then echo 'The krita app is not available.'; fi
...
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes bridge-utils cpu-checker libvirt-clients libvirt-daemon-system qemu-kvm qemu-system virtinst virt-manager; fi
...
# FTP/HTTP/BitTorrent client.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes lftp; fi
...
# Office suite.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes aspell-en aspell-nl libreoffice libreoffice-l10n-nl; fi
...
# Find files.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes locate; fi
...
# Python static typing.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes mypy; fi
...
# Administrative operations.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes nautilus-admin; fi
if $DESKTOP_ENVIRONMENT && $RPM_SYSTEM; then echo 'The nautilus-admin app is not available.'; fi
...
# Network MAPper.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes nmap; fi
...
# NTFS support.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes ntfs-3g; fi
...
# Programming language.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes pycodestyle python3-pycodestyle python3-autopep8 python3-pip python-is-python3; fi
...
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes rpm; fi
...
# Shell script linter.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes shellcheck; fi
...
# Audio-cd ripper and player.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes sound-juicer; fi
if $DESKTOP_ENVIRONMENT && $RPM_SYSTEM; then echo 'The sound-juicer app is not available.'; fi
...
# Spice agent.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes spice-vdagent; fi
...
# Music and podcasts.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes spotify-client; fi
...
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes ssh; fi
...
# Quick preview.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes gnome-sushi; fi
...
# Bash completion.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes bash-completion; fi
if $RPM_SYSTEM; then sudo dnf remove --assumeyes bash-completion; fi

# Install teamviewer on *
# Remote desktop.
# Direct deb download: wget https://download.teamviewer.com/download/linux/teamviewer_amd64.deb
...
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes teamviewer; fi
...
# E-mail and news.
if $DESKTOP_ENVIRONMENT && $DEBIAN; then sudo apt-get remove --purge --assume-yes thunderbird-l10n-nl; fi
if $DESKTOP_ENVIRONMENT && $UBUNTU; then sudo apt-get remove --purge --assume-yes thunderbird-locale-nl; fi
...
# Display directory tree.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes tree; fi
...
# Uncomplicated FireWall.
sudo ufw disable
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes gufw; fi
...
# USB utilities.
# This package contains the lsusb utility.
if $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes usbutils; fi
...
# Virtualization.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes virtualbox virtualbox-ext-pack virtualbox-guest-additions-iso; fi
...
# Remove vlc from *
# Multimedia player.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes vlc; fi
...
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes code; fi
...
# Web console.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes webmin; fi
...
# Run Windows applications.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes wine winetricks playonlinux; fi
...
# Download videos.
if $DESKTOP_ENVIRONMENT && $APT_SYSTEM; then sudo apt-get remove --purge --assume-yes youtubedl-gui; fi
